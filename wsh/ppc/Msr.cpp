// wsh/ppc/Msr.cpp - Machine State Register
//   Written by mkwcat
//
...
bool Msr::EnableInterrupts() noexcept {
  Msr modMsr = MoveFrom();
  bool prev = modMsr.EE;
  modMsr.EE = true;
  modMsr.MoveTo();
...
bool Msr::DisableInterrupts() noexcept {
  Msr modMsr = MoveFrom();
  bool prev = modMsr.EE;
  modMsr.EE = false;
  modMsr.MoveTo();
...
void Msr::RestoreInterrupts(bool prev) noexcept {
  Msr modMsr = MoveFrom();
  if (prev == modMsr.EE) {
    return;
  }
